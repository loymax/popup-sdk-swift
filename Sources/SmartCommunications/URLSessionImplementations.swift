...
public typealias SmartCommunicationsAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, SmartCommunicationsAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: SmartCommunicationsAPIChallengeHandler?
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = SmartCommunicationsAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
        ...
        do {
            ...
            let dataTask = urlSession.dataTaskFromProtocol(with: request) { data, response, error in
                ...
                self.logResponse(request: request, response: response, data: data, error: error)
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in SmartCommunicationsAPI.customHeaders {
    ...
    private func logRequest(_ request: URLRequest) {
            var log = "➡️ REQUEST\n"
            log += "URL: \(request.url?.absoluteString ?? "No URL")\n"
            log += "Method: \(request.httpMethod ?? "No Method")\n"
            ...
            log += "➡️ END REQUEST\n"
            Logger.shared.log(log, isRequest: true)
        }
        ...
        private func logResponse(request: URLRequest, response: URLResponse?, data: Data?, error: Error?) {
            ...
            log += "⬅️ END RESPONSE\n"
            Logger.shared.log(log, isRequest: false)
        }
...
open class Logger {
    ...
    private var logs: [(message: String, isRequest: Bool)] = []
    private var observers: [((String, Bool) -> Void)] = []

    private init() {}

    // Add a log entry and notify observers
    public func log(_ message: String, isRequest: Bool) {
        let formattedMessage = "\(Date()): \(message)"
        logs.append((formattedMessage, isRequest))
        notifyObservers(formattedMessage, isRequest: isRequest)
    }

    // Add an observer for new logs
    public func addObserver(_ observer: @escaping (String, Bool) -> Void) {
        observers.append(observer)
    }

    // Notify all observers
    private func notifyObservers(_ message: String, isRequest: Bool) {
        for observer in observers {
            observer(message, isRequest)
        }
    }
}
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.SmartCommunications.\(UUID().uuidString)")
                }
